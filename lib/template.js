...
function renderHandlebars(viewFiles, viewModel, callback) {
  var hbTemplatePath = Path.join(__dirname, '../templates/handlebars/');
...
function renderStaticHtml(viewFiles, callback) {
  ...
  if (viewFiles.length > 1) {
    console.warn('TEMPLATES: More than one possible HTML view file found, will take main or index.');
  ...
  if (htmlFile) {
    FS.readFile(htmlFile, function (err, html) {
      if (!err) {
        ...
        console.error('TEMPLATES: Error reading HTML file ' + htmlFile, err);
        callback && callback();
      }
    });
  }
  else {
    console.warn('TEMPLATES: Could not find appropriate main HTML view file, please rename your main file to main.html or index.html.');
    callback && callback();
  }
}
