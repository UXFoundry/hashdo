...
function sass(sassFiles, callback) {
  ...
  if (sassFiles.length > 1) {
    console.warn('Styles-Lib: More than one possible SASS file found, will take main or global.');
  ...
  if (sassFile) {
    Sass.render({
      ...
      if (!err) {
        ...
        console.error('Styles-Lib: Error rendering SASS file ' + sassFile, err);
        callback && callback();
      }
    });
  }
  else {
    console.warn('Styles-Lib: Could not find appropriate main SASS file, please rename your main stylesheet to main.scss or global.scss.');
    callback && callback();
  }
}
...
function less(lessFiles, callback) {
  ...
  if (lessFile) {
    Less.render(FS.readFileSync(lessFile).toString(), {
      ...
      if (!err) {
        ...
        console.error('Styles-Lib: Error rendering LESS file ' + lessFile, err);
        callback && callback();
      }
    });
  }
  else {
    console.warn('Styles-Lib: Could not find appropriate main LESS file, please rename your main stylesheet to main.less or global.less.');
    callback && callback();
  }
}
...
function css(styleSheets, callback) {
  ...
  if (stylesConcat.length > 0) {
    callback && callback(stylesConcat);
  }
  else {
    callback && callback();
  }
