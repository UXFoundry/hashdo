...
module.exports = {
  init: function (baseUrl, cardsDirectory) {
    ...
    console.log('PACKS: Cards will be loaded from %d packs found in %s.', packs.length, cardsDirectory);
        ...
        if (!packInfo.hidden) {
          ...
          for (var j = 0; j < packCards.length; j++) {
            ...
            cardPacks[packName].cards[cardName] = {
              ...
              icon: card.icon || _.trimRight(baseUrl, '/') + '/' + packName + '/' + cardName + '/icon.png',
              baseUrl: _.trimRight(baseUrl, '/') + '/' + packName + '/' + cardName,
              inputs: card.inputs
            };
          }
        }
