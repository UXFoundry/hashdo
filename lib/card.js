        ...
        if (options) {
          ...
          if (options.clientStateSupport || options.clientAnalyticsSupport) {
            ...
            if (options.clientStateSupport) {
              start += 'var baseUrl = "' + _.trimRight(process.env.BASE_URL || '', '/') + '";';
              start += 'var firebaseUrl = "' + _.trimRight(process.env.FIREBASE_URL || '', '/') + '";';
...
/**
  ...
  */
function generateCard(options, callback) {
  var elementId = Cuid();
...
var card = {
  ...
  render: function (options, callback) {
    if (!options) {
      throw new Error('You must provide an options object to render the card.');
    }
    ...
    FS.stat(packPath, function (err) {
      if (!err) {
        ...
        var Card = getCard(Path.join(packPath, options.cardName));
        ...
        if (Card) {    
          Async.waterfall([
              ...
              function (params, viewModel, clientLocals, cb) {
                ...
                generateCard(generateOptions, function (html) {
                  cb(null, html);
                });
              }
            ],
          ...
          callback && callback('Card not found within ' + options.packName);
        }
      }
      else {
  ...
  /**
    * Provides the functionality to call a card's webhook functionality and save the state.
    *
    * @method webhook
    * @async
    ...
    FS.stat(packPath, function (err) {
      if (!err) {
        ...
        if (Card && Card.webHook) {
          ...
          callback && callback('No card found or web hook not avaialble.');
        }
      }
      else {
        // No card pack.
